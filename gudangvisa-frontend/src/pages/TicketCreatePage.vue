...
import type { TrackingTicket } from '../types';
...
const created = ref<TrackingTicket | null>(null);
...
<template>
  <div class="max-w-lg">
    <div class="mb-8">
      ...
      <h1 class="text-[28px] font-bold text-heading">Create New Ticket</h1>
    </div>
    ...
    >
      <div class="flex flex-col gap-1.5">
        ...
        <select
          id="tc-client"
          v-model="clientId"
          class="w-full px-3.5 py-2.5 text-sm text-heading bg-panel-light border border-edge rounded-lg outline-none cursor-pointer appearance-none focus:border-indigo-500 focus:ring-3 focus:ring-indigo-500/12"
          required
        >
      ...
      <div class="flex flex-col gap-1.5">
        ...
        <p class="text-xs text-subtle mt-0.5">
          Select the immigration document service for this ticket.
        </p>
      </div>

      <button
        type="submit"
        class="w-full flex items-center justify-center gap-2 px-5 py-2.5 text-sm font-semibold rounded-lg bg-indigo-500 text-white hover:bg-indigo-600 transition-colors disabled:opacity-50 cursor-pointer"
    ...
    >
      ...
      <h2 class="text-2xl font-bold text-heading mb-2">Ticket Created!</h2>
      <p class="text-[15px] text-subtle mb-5">
        Share this tracking code with the client:
      </p>
      <div class="mb-6">
        <code
          class="text-[28px] font-bold text-indigo-400 bg-indigo-500/10 px-6 py-3 rounded-xl tracking-widest"
      ...
      <div class="flex justify-center gap-3">
        ...
        <button
          ...
          @click="router.push(`/tickets/${created.id}`)"
        >
          View Ticket
        </button>
      </div>
