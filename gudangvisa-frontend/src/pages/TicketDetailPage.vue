    ...
    <template v-if="ticket">
        ...
        <div class="flex justify-between items-start gap-3 flex-wrap mb-4">
          <div>
            <div class="flex items-center gap-2">
              <code
                class="text-lg text-indigo-400 bg-indigo-500/10 px-3 py-1 rounded-lg font-mono font-bold"
                >{{ ticket.trackingCode }}</code
              >
              <button
                class="p-1.5 rounded-md text-subtle hover:text-indigo-400 hover:bg-indigo-500/10 transition-colors cursor-pointer"
                title="Copy Code"
                @click="
                  copyToClipboard(
                    ticket.trackingCode,
      ...
      <div class="bg-panel border border-edge rounded-2xl p-6 mb-6">
        ...
        <div v-if="showStatusForm" class="flex flex-col gap-3">
          ...
          <input
            v-model="descPublic"
            type="text"
            class="w-full px-3.5 py-2.5 text-sm text-heading bg-panel-light border border-edge rounded-lg outline-none focus:border-indigo-500 placeholder:text-subtle"
            placeholder="Public description (required) *"
            required
          />
          <input
            v-model="descInternal"
            type="text"
            class="w-full px-3.5 py-2.5 text-sm text-heading bg-panel-light border border-edge rounded-lg outline-none focus:border-indigo-500 placeholder:text-subtle"
          ...
          <div class="flex gap-2 justify-end">
            ...
            <button
              class="px-4 py-2 text-sm font-semibold rounded-lg bg-indigo-500 text-white hover:bg-indigo-600 transition-colors disabled:opacity-50 cursor-pointer"
      ...
      <div class="bg-panel border border-edge rounded-2xl p-6 mb-6">
        ...
        >
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div class="flex flex-col gap-1.5">
              ...
              <input
                v-model="docName"
                type="text"
                class="w-full px-3 py-2 text-sm text-heading bg-dark border border-edge rounded-lg outline-none focus:border-indigo-500 placeholder:text-subtle"
            ...
            <div class="flex flex-col gap-1.5">
              ...
              <select
                v-model="docType"
                class="w-full px-3 py-2 text-sm text-heading bg-dark border border-edge rounded-lg outline-none cursor-pointer appearance-none focus:border-indigo-500"
              >
          ...
          />
          <button
            class="w-full px-4 py-2.5 text-sm font-semibold rounded-lg bg-indigo-500 text-white hover:bg-indigo-600 transition-colors disabled:opacity-50 cursor-pointer"
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              ...
              <p class="text-xs text-subtle">
                {{ doc.docType }} · {{ doc.isPublic ? 'Public' : 'Private' }}
              </p>
            </div>
            <a
              ...
              target="_blank"
              class="text-sm text-indigo-400 hover:underline mr-2"
              >Download</a
            >
            <button
              v-if="auth.isAdmin"
