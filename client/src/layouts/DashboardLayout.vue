...
<template>
  <div class="flex min-h-screen bg-dark transition-colors duration-300">
    ...
    <!-- Sidebar -->
    <aside
      class="w-64 min-w-64 bg-panel border-r border-edge flex flex-col sticky top-0 h-screen overflow-y-auto transition-all duration-300 z-50 max-md:fixed max-md:left-0 max-md:top-0 max-md:bottom-0"
      :class="[
    ...
    >
      ...
      <div class="px-5 py-6 border-b border-edge">
        <router-link
          to="/dashboard"
          class="flex items-center gap-2.5 text-xl font-bold text-heading"
        >
          <img
      ...
      <nav class="flex-1 px-3 py-5 flex flex-col gap-1">
        <p
          class="px-3 mb-1.5 text-[11px] font-semibold uppercase tracking-wider text-subtle"
        >
          Menu
        </p>
        <router-link
          ...
          :class="
            isActive(item.path)
              ...
              : 'text-body hover:bg-panel-light hover:text-heading'
          "
        >
          <!-- Dashboard icon -->
          <svg
          ...
          <!-- Applications icon -->
          <svg
            v-else-if="item.icon === 'applications'"
            xmlns="http://www.w3.org/2000/svg"
            width="20"
            height="20"
            viewBox="0 0 24 24"
          ...
          <span
            v-if="
              item.icon === 'applications' && applicationStore.totalCount > 0
            "
      ...
      <div class="px-4 py-4 border-t border-edge flex items-center gap-2">
        <router-link
          ...
          @click="isSidebarOpen = false"
          title="View profile"
          class="flex items-center gap-2.5 flex-1 min-w-0 -m-1 p-1 rounded-lg hover:bg-panel-light transition-colors"
        >
          <div
            class="w-9 h-9 rounded-lg bg-red-500/12 text-red-400 flex items-center justify-center font-bold text-sm shrink-0"
          >
            {{ auth.user?.fullName?.charAt(0) ?? 'U' }}
          </div>
          <div class="min-w-0">
            <p class="text-[13px] font-semibold text-heading truncate">
              {{ auth.user?.fullName ?? 'User' }}
            </p>
            <p class="text-[11px] text-subtle uppercase tracking-wider">
              {{ auth.user?.role ?? '' }}
            </p>
          </div>
        </router-link>
