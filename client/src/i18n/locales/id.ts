export default {
  nav: {
    services: 'Layanan',
    faq: 'Pertanyaan Umum',
    about: 'Tentang Kami',
    contact: 'Hubungi CS',
  },
  hero: {
    brand: 'GudangVisa',
    title: 'Lacak Dokumen Anda',
    subtitle:
      'Masukkan kode pelacakan Anda untuk melihat status saat ini dan riwayat lengkap dokumen imigrasi Anda.',
  },
  tracking: {
    cta: 'Lacak Dokumen',
    placeholder: 'Masukkan nomor resi (GV-2026-xxxxx)',
    note: 'Masukkan nomor resi yang Anda terima saat pendaftaran untuk melihat status terkini dan mengunduh dokumen yang sudah selesai — tanpa perlu login.',
    empty: 'Nomor resi wajib diisi.',
  },
  services: {
    title: 'Layanan Utama Kami',
    subtitle:
      'Kami menyediakan jasa pengurusan dokumen imigrasi yang cepat, andal, dan transparan.',
    items: [
      {
        title: 'Pengurusan Visa',
        desc: 'Proses visa turis, bisnis, dan sosial dengan pelacakan transparan dari awal hingga selesai.',
      },
      {
        title: 'KITAS / KITAP',
        desc: 'Pengurusan izin tinggal terbatas dan tetap untuk tenaga kerja asing, penyatuan keluarga, atau investor.',
      },
      {
        title: 'Layanan Paspor',
        desc: 'Membantu pembuatan paspor baru, perpanjangan masa berlaku, serta penanganan paspor rusak atau hilang.',
      },
    ],
  },
  faq: {
    title: 'Pertanyaan Umum (FAQ)',
    subtitle:
      'Punya pertanyaan? Kami punya jawabannya. Jika Anda tidak menemukan apa yang Anda cari, hubungi CS kami.',
    items: [
      {
        q: 'Bagaimana cara melacak dokumen imigrasi saya?',
        a: 'Masukkan kode pelacakan yang Anda terima pada kolom pelacakan di bagian atas halaman ini, lalu klik tombol "Lacak" untuk melihat status dan riwayat dokumen Anda.',
      },
      {
        q: 'Jenis dokumen apa saja yang bisa diproses di sini?',
        a: 'Kami menangani pengurusan visa, KITAS/KITAP, paspor, serta berbagai dokumen imigrasi lainnya.',
      },
      {
        q: 'Berapa lama tautan pelacakan dokumen tetap aktif?',
        a: 'Tautan pelacakan tetap aktif selama dokumen Anda masih dalam proses dan beberapa waktu setelah selesai, sehingga Anda dapat mengunduh hasil akhir.',
      },
    ],
  },
  about: {
    eyebrow: 'TENTANG KAMI',
    company: 'PT. Gudang Konsultan Indonesia',
    tagline: 'Agensi utama Anda untuk semua kebutuhan liburan dan bisnis di Bali.',
    body: [
      'Gudang Visa Indonesia didirikan pada tahun 2023 dan berlokasi di Bali. Kami dikelola oleh tenaga profesional dan berpengalaman di bidangnya untuk memberikan pelayanan terbaik dalam pengurusan Visa, KITAS, Tiket Pesawat, hingga pendirian bisnis Anda dengan dukungan tim hukum kami.',
      'Kami memprioritaskan transparansi, komunikasi yang jelas, dan layanan pelanggan yang luar biasa untuk membangun hubungan jangka panjang dengan klien. Kami juga menawarkan harga yang sangat kompetitif dibanding agensi lain di Bali.',
    ],
    visionTitle: 'Visi Kami',
    vision:
      'Menjadi perusahaan profesional yang menyediakan layanan terbaik dengan akurasi dalam waktu yang cepat dan tepat.',
    missionTitle: 'Misi Kami',
    mission:
      'Memberikan solusi dan ide terbaik untuk setiap kebutuhan dan permasalahan klien dengan layanan yang cepat, ramah, dan profesional.',
  },
  whyUs: {
    title: 'Mengapa Memilih Kami?',
    items: [
      {
        title: 'Perusahaan Terpercaya',
        desc: 'Kami adalah perusahaan terpercaya yang memiliki legalitas hukum yang jelas.',
      },
      {
        title: 'Harga Terbaik',
        desc: 'Kami menawarkan harga terbaik yang sangat kompetitif di pasar Bali.',
      },
      {
        title: 'Respon Cepat',
        desc: 'Komunikasi yang transparan dan respon kilat untuk setiap kebutuhan Anda.',
      },
    ],
  },
  footer: {
    navTitle: 'Navigasi',
    servicesTitle: 'Layanan',
    nav: {
      services: 'Layanan',
      faq: 'Pertanyaan Umum',
      about: 'Tentang Kami',
    },
    services: {
      checkStatus: 'Cek Status Dokumen',
      guide: 'Panduan Visa',
      express: 'Layanan Ekspres',
    },
    rights: 'Hak cipta dilindungi.',
    privacy: 'Kebijakan Privasi',
    terms: 'Syarat Layanan',
  },
};
