...
async function loadApplication(): Promise<void> {
  ...
  try {
    ...
    notify.error(err instanceof Error ? err.message : 'Failed to load application');
...
async function handleStatusUpdate(): Promise<void> {
  if (!application.value || !descPublic.value.trim()) return;
  ...
  try {
    ...
    // Sync local document statuses to match the updated application status
      ...
      <div class="bg-panel border border-edge rounded-2xl p-6 mb-6">
        ...
        >
          ...
          <p class="text-sm text-emerald-300">
            This application has been completed. No further status updates are
            allowed.
          </p>
        </div>
